...
-(void) drawTextInRect:(CGRect)requestedRect
{
    ...
    CGPathAddRect(leftColumnPath, NULL ,CGRectMake(-15 , -15 ,self.bounds.size.width , self.bounds.size.height));
    ...
    CGContextRef context1 = UIGraphicsGetCurrentContext();
      ...
      //2.a.1创建一条绘图的路径
        ...
        //2.a.2把绘图信息添加到路径里
        CGPathMoveToPoint(path, NULL, self.bounds.size.width-70, 20);
    
        CGPathAddLineToPoint(path, NULL, self.bounds.size.width-55, 30);
    
        CGPathAddLineToPoint(path, NULL, self.bounds.size.width-70, 40);
         //2.a.3把路径添加到上下文中
        //把绘制直线的绘图信息保存到图形上下文中
...
-(void)handleTap:(UIGestureRecognizer*) recognizer
{
    ...
    if ([self.delegate respondsToSelector:@selector(Rlabel:)])
    {
        [self.delegate Rlabel:self];
    }
