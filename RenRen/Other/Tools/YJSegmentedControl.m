...
- (void)AddSegumentArray:(NSArray *)SegumentArray{
    ...
    for (int i = 0; i < SegumentArray.count; i++) {
        ...
        if (i == 0) {
            self.buttonDownView =[[UIView alloc]initWithFrame:CGRectMake(i * witdthFloat, self.bounds.size.height - 3, witdthFloat,3)];
...
-(void)selectTheSegument:(NSInteger)segument{
    ...
    if (selectSeugment != segument) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [self.buttonDownView setFrame:CGRectMake(segument * witdthFloat,self.bounds.size.height - 3, witdthFloat, 3)];
        }];
