...
//    calendarView.currentYear    = _currentYear;
    calendarView.signItemArray    = _signArray;//签到天数显示
    calendarView.getPASS          = self.passStr;
...
- (void)monthOnclick:(NSInteger)lastOrNext
{
    if (lastOrNext) {
        if (_currentScrollIndexRow < TotalNumber - 1)
        {
            ...
            JLCalendarItem *cell =(JLCalendarItem*) [_collectionView cellForItemAtIndexPath:index];
            cell.signItemArray =self.signArray;
